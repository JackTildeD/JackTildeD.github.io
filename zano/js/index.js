    ...
    for (const elem of document.getElementsByClassName("landing--fith")) {
        ...
        if (obj.left <= x && x <= obj.right && obj.top <= y && y <= obj.bottom)
            window.location.href = `${window.location.href
                .split("/")
                .slice(0, -1)
                .join("/")}/codex.html?g=${encodeURIComponent(
                Math.max(0, indexClick.end - 1),
            )}#top`;
    }
    ...
    for (const elem of document.getElementsByClassName("landing--seventh")) {
        ...
        if (obj.left <= x && x <= obj.right && obj.top <= y && y <= obj.bottom)
            window.location.href = `${window.location.href
                .split("/")
                .slice(0, -1)
                .join("/")}/codex.html?g=${encodeURIComponent(0)}#top`;
    }
    return;
};
...
const indexMain = async () => {
    ...
    for (const elem of document.querySelectorAll(
        ...
        elem.style.backgroundImage = `url('${IMG_ROOT}/${indexMain.pages[
            indexMain.end
    ...
    for (const elem of document.querySelectorAll(
        ...
        elem.style.backgroundImage = `url('${IMG_ROOT}/${indexMain.pages[
            Math.max(0, indexMain.end - 1)
        ].filename.replace(/\.[^.]*$/, "")}.webp')`;
    for (const elem of document.querySelectorAll(
        ".landing--sixth .cuboid__face--front",
    ))
        elem.style.backgroundImage = `url('${IMG_ROOT}/${indexMain.pages[
            Math.max(0, indexMain.end - 2)
        ].filename.replace(/\.[^.]*$/, "")}.webp')`;
    for (const elem of document.querySelectorAll(
        ".landing--seventh .cuboid__face--front",
    ))
        elem.style.backgroundImage = `url('${IMG_ROOT}/${indexMain.pages[0].filename.replace(
            /\.[^.]*$/,
            "",
        )}.webp')`;
    return;
