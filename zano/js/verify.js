...
Note: to use "\\", "<", or ">" in your document replace them with ${""
}"\\\\", "\\<", or "\\>".
...
for (const elem of document.getElementsByClassName("button-verify"))
    elem.addEventListener("click", () => {
        let html, info;
        for (
            const elem of document.getElementsByClassName(
                "text-input"
            )
        ) {
            ...
            info = analyze(html).concat(html.length).concat(Math.abs(t2 - t1));
        }
        if (html) {
            for (
                const elem of document.getElementsByClassName(
                ...
                elem.innerHTML = `<ul>
    <li>
        Opening angle brakets: ${info[0]} ${
            info[0] == info[1]? "OK" : "MISMATCH"
        }
    </li>
    <li>
        Closing angle brakets: ${info[1]} ${
            info[0] == info[1]? "OK" : "MISMATCH"
        }
    </li>
    <li>
    ...
    <li>
        Closing preformatted (pre) tags: ${
            info[7]} ${info[6] == info[7]? "OK" : "MISMATCH"
        }
    </li>
    <li>
    ...
    <li>
        Parse time: ${info[9]} ms ${
            ...
            for (
                const elem of document.getElementsByClassName("verify-output")
            )
                elem.innerHTML = html;
        }
        update();
