...
document.addEventListener(
    ...
    () => {
        for (
            ...
            if (
                !document.getElementsByClassName("common-banner__star").length
            ) {
                for (let i = 0; i < 200; i++) {
...
const getCodex =
    async x => await (await fetch(`${ZANO_ROOT}/codex/${x}`)).text();
...
const getDirectory = async () =>
    (await (await fetch("directory.txt")).text()).split("\n").filter(
        ...
        x[0] === "" ? {filename: null, part: x[1], date: null, title: x[3]}
            : {filename: x[0], part: parseInt(x[1]), date: x[2], title: x[3]}
    );
...
const deleteCookie = varName => {
    document.cookie =
        `${varName}=; expires=${new Date(0).toUTCString()}; path=/`;
    return;
};
...
const deleteCookies = () => {
    ...
    for (var i = 0; i < allCookies.length; i++)
        document.cookie =
            `${allCookies[i]}=; expires=${new Date(0).toUTCString()}; path=/`;
    return;
};
...
const parseCodex = x => {
    ...
    while (true) {
        ...
        if (/^\s*</.test(groups[2])) {
            ...
            const src = groups[2].replace(
                    /(^\s*<\s*\S*\s*)|(>.*)|(["'])/gis, ""
                ).split(" ")[0].replace("~", IMG_ROOT);
            const additional = classes.replace(
                    /^\s*\S*\s*/gis, ""
                );
            ...
            if ((/^\s*<\s*MODAL\s?/i).test(groups[2])) {
                ...
                output +=
                    `<img class="modal__target ${additional}" src="${src}">`;
                continue;
            }
